<?php
/*
 * Xataface PDF Editor Widget Module v 0.1
 ...
 *
 */
class Dataface_FormTool_pdfeditor {
	...
	function &buildWidget($record, &$field, $form, $formFieldName, $new=false){
		...
		$editorConfig = array();
		if ( @$widget['pdfeditor'] ){
			$editorConfig = $widget['pdfeditor'];
		}
		
		$atts = array(
			'class' => 'xf-pdfeditor',
			'data-xf-pdfeditor-base-path' => $mod->getBaseURL().'/js/xataface/modules/pdfreports',
			'data-xf-pdfeditor-config' => json_encode($editorConfig)
		);
		...
		$jt->import('xataface/modules/pdfreports/pdfeditor-widget.js');
